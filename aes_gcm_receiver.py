...
def AES_decrypt(byte_array, key, length):
...
if __name__ == '__main__':
    ...
    file_size = 1799168     #test.mp4 = 9682329
    file_name = 'test1.mp4'

    # Get file size
    data, Source_addr = receiver.recvfrom(256)
    file_size = int(data)
    ...
    while not recv_size == file_size:
        if file_size - recv_size > 256:
           ...
           # Decrypted_array += byte_array
            #解密    
            for i in range(0,16):
                Decrypted_array += AES_decrypt(byte_array[array_offset:array_offset+16], key, 16)
                array_offset += 16

            byte_array.clear()
        else:
            data, Source_addr = receiver.recvfrom(file_size - recv_size)
            recv_size += len(data)
            #print(recv_size)
            byte_array += data
            ...
            while(1):
                if(remain_size < 16):
                    Decrypted_array += AES_decrypt(byte_array[array_offset:array_offset+remain_size], key, remain_size)
                    break
                else:
                    Decrypted_array += AES_decrypt(byte_array[array_offset:array_offset+16], key, 16)
                    array_offset += 16
