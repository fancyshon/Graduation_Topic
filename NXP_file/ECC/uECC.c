...
int uECC_mult(uint8_t *public_key,
                  uint8_t *private_key,
                  uECC_Curve curve) {
#if uECC_VLI_NATIVE_LITTLE_ENDIAN
    uECC_word_t *_private = (uECC_word_t *)private_key;
...
int uECC_shared_secret(const uint8_t *public_key,
...
#else
    uECC_vli_bytesToNative(_private, private_key, num_bytes);
    ...
    /* Regularize the bitcount for the private key so that attackers cannot use a side channel
       attack to learn the number of leading zeros. */
    carry = regularize_k(_private, tmp1, tmp2, curve);
    ...
    EccPoint_mult(result, _public, p2[!carry], initial_Z, curve->num_n_bits + 1, curve);
#if uECC_VLI_NATIVE_LITTLE_ENDIAN
    bcopy((uint8_t *) secret, (uint8_t *) _public, num_bytes);
#else
    uECC_vli_nativeToBytes(secret, num_bytes, result);
    uECC_vli_nativeToBytes(secret + num_bytes, num_bytes, result + num_words);
#endif
